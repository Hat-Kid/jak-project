...
void add_ambients_from_json(const nlohmann::json& json,
  ...
  for (const auto& ambient_json : json) {
    ...
    std::unordered_map<std::string, float> keyframes;
    if (ambient_json.find("keyframes") != ambient_json.end()) {
      for (const auto& [key, value] : ambient_json.at("keyframes").items()) {
        ASSERT_MSG(value.is_number_float(),
                   fmt::format("keyframes: value {} must be a float!", value.get<float>()));
        keyframes.emplace(key, value);
      }
    ...
    if (ambient_json.find("lump") != ambient_json.end()) {
      for (const auto& [key, value] : ambient_json.at("lump").items()) {
        float keyframe = get_custom_keyframe(key);
        if (value.is_string()) {
          auto value_string = value.get<std::string>();
          if (!value_string.empty() && value_string[0] == '\'') {
            ...
            ambient.res_lump.add_res(std::make_unique<ResString>(key, value_string, keyframe));
          }
          continue;
        }
        if (value.is_array()) {
          ambient.res_lump.add_res(res_from_json_array(key, value, dts, keyframe));
        }
      }
    }
